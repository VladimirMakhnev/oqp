#!/bin/bash
# Numerical gradient verification for ROKS-MRSF (BHHLYP)
...
echo "=== Numerical Gradient: ROKS-MRSF (BHHLYP) ==="
...
for tag in p1 m1; do
    ...
    cat > /tmp/num_roks_mrsf_${tag}.inp << EOF
[input]
system=
...
EOF
    ...
    openqp /tmp/num_roks_mrsf_${tag}.inp 2>&1 > /dev/null
done
...
# Extract energies
with open('/tmp/num_roks_mrsf_p1.log') as f:
    for line in f:
        if 'PyOQP state 1' in line and '-276' in line:
            E_p1 = float(line.split()[3])
            break

with open('/tmp/num_roks_mrsf_m1.log') as f:
    for line in f:
        if 'PyOQP state 1' in line and '-276' in line:
...
anal = -0.481739  # From test_roks_mrsf_grad.sh GAMESS reference

print("")
print("=== ROKS-MRSF (BHHLYP) Results ===")
