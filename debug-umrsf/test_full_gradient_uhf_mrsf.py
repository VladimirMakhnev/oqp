#!/usr/bin/env python3
"""
Full numerical gradient verification for UHF-MRSF
...
def extract_analytical_gradient(log_file, state=1):
    """Extract analytical gradient from log file"""
    gradients = []
    with open(log_file) as f:
        lines = f.readlines()

    # Find gradient section after "Final Gradient"
    in_gradient = False
    for i, line in enumerate(lines):
        if 'PyOQP electronic gradients' in line:
            # Next line is "PyOQP state N", then atom gradients
            for j in range(i+1, min(i+20, len(lines))):
                if f'PyOQP state {state}' in lines[j]:
                    # Read next 6 lines for 6 atoms
...
def numerical_gradient_component(atom_idx, coord_idx, coords):
    """Calculate numerical gradient for one component"""
    coord_names = ['X', 'Y', 'Z']

    # Plus displacement
    coords_p = [list(c) for c in coords]
    coords_p[atom_idx][coord_idx + 1] += h  # +1 because first element is atomic number
    ...
    # Run calculations
    inp_p = f'numgrad_mrsf_a{atom_idx}_{coord_names[coord_idx]}_p.inp'
    inp_m = f'numgrad_mrsf_a{atom_idx}_{coord_names[coord_idx]}_m.inp'
    ...
    # Numerical gradient in Hartree/Bohr
...
def main():
    ...
    # Calculate numerical gradients for all components
